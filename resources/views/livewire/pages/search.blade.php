...
use function Livewire\Volt\{layout, state, usesPagination};
...
$search = function () {
    ...
    $this->cards = Card::search($this->query)->options([
        'query_by' => 'name, hp'
    ])->get();
};
...
<div>
    <div class="py-12 px-4 space-y-8">
        <form wire:submit="search" class="relative max-w-md mx-auto rounded-full ">
            ...
            <x-form.input wire:keyup.debounce="search" wire:model="query" type="text" placeholder="Search by name, HP and more" class="w-full py-4 rounded-full md:text-lg md:py-4 md:px-6" />
